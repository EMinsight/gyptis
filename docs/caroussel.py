...
def validate_tooltip(tooltip: str) -> str:
    ...
    if len(sanitized) > 500:
        logger.warning(f"Tooltip exceeds 500 characters, truncating: {sanitized[:50]}...")
...
def parse_rst_content(lines: List[str]) -> Tuple[List[str], List[str], List[str]]:
    ...
    for line_num, line in enumerate(lines, 1):
        try:
            ...
            # Extract image (only within thumbnail containers)
                ...
                if image and current_tooltip:
                    ...
                    if "_thumb" in image or "thumb/" in line.lower():
                        ...
                        logger.debug(f"Line {line_num}: Skipped non-thumbnail image: {image[:50]}")
            ...
            # Extract title from sphx-glr-thumbnail-title div
                ...
                title_text = line.replace(config.TITLE_PREFIX, "").replace("</div>", "").strip()
                if title_text:
                    try:
                        title = validate_title(title_text)
                        titles.append(title)
                        logger.debug(f"Line {line_num}: Extracted title from thumbnail-title div")
                    except ValidationError:
                        titles.append("Example")
                        logger.warning(f"Line {line_num}: Failed to validate title, using fallback")
                else:
                    titles.append("Example")
            ...
            # Detect when we exit a thumbnail container (fallback)
                ...
                if current_tooltip and len(tooltips) > len(titles):
                    titles.append("Example")
                    logger.debug(f"Line {line_num}: Added default title for incomplete thumbnail")
                in_thumbnail = False
                current_tooltip = None
    ...
    logger.info(f"Extraction complete: {len(images)} images, {len(titles)} titles, {len(tooltips)} tooltips")
...
def validate_extracted_data(
    ...
    logger.warning(f"Data length mismatch: images={len(images)}, titles={len(titles)}, tooltips={len(tooltips)}")
...
def inject_carousel_into_html(html_file: str, carousel_html: str) -> None:
    ...
    try:
        ...
        if not placeholder_found:
            logger.warning(f"Placeholder '{config.CAROUSEL_PLACEHOLDER}' not found in {html_file}")
            logger.info("Carousel generated but not injected into HTML")
            return
        
        # Replace placeholder with carousel
        modified_lines = [
            line.replace(config.CAROUSEL_PLACEHOLDER, carousel_html) 
            for line in lines
        ]
        ...
        logger.info("Carousel generated but could not be injected (HTML file may not be built yet)")
    except Exception as e:
